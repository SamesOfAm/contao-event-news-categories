...
#[AsFrontendModule('event_category_filter', category: 'miscellaneous', template: 'frontend_module/event_category_filter')]
class EventCategoryFilterController extends AbstractFrontendModuleController
{
    ...
    {
        $allowedIds = StringUtil::deserialize($model->allowedEventCategories, true);

        if (!empty($allowedIds)) {
            $placeholders = implode(',', array_fill(0, count($allowedIds), '?'));
            $rows = $this->connection->fetchAllAssociative(
                "SELECT id, name, cssClass FROM tl_event_category WHERE id IN ($placeholders) ORDER BY name",
                array_values($allowedIds)
            );
        } else {
            $rows = $this->connection->fetchAllAssociative(
                'SELECT id, name, cssClass FROM tl_event_category ORDER BY name'
            );
        }
